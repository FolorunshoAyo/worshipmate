...
if ($(".modal-uploader").length) {
    ...
    modalUploader.on("addedfile", (function(e) {
        ...
        if(e.type.search("video") !== -1){
            e.previewElement.querySelector("textarea").setAttribute("placeholder", "Describe this video ...");
            const videoBuffer = e;

            let url = window.URL.createObjectURL(videoBuffer);

            const html = "<video autoplay controls src='" + url + "'></video>"
        
            e.previewElement.querySelector(".preview").innerHTML = html;
        }else{
            e.previewElement.querySelector("textarea").setAttribute("placeholder", "Describe this photo ...")
        }

        e.previewElement.id = "uploaded-file-" + uploadCount,
        e.previewElement.querySelector("textarea").setAttribute("name", "uploaded_file_textarea_" + uploadCount),
        e.previewElement.querySelector("textarea").id = "uploaded-file-textarea-" + uploadCount,
            ...
            for (var l = e[t], r = Math.round(Math.min(maxSteps, Math.max(minSteps, l.size / bytesPerStep))), a = 0; a < r; a++) {
                ...
                setTimeout(function(e, t, l) {
                    return function() {
                        ...
                        o.emit("uploadprogress", e, e.upload.progress, e.upload.bytesSent),
                        100 == e.upload.progress && (e.status = Dropzone.SUCCESS,
                        o.emit("success", e, "success", null),
                        o.emit("complete", e),
                        o.processQueue())
                    }
                }(l, r, a), n)
